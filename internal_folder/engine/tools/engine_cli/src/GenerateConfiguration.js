...
class GenerateConfiguration {
  ...
  run(args) {
    ...
    if (args.watch) {
      ...
      childProcess.execSync(
        `watchman -- trigger-del ${rootPath} engine`,
      );
      childProcess.execSync(
        `watchman -- trigger ${rootPath} engine ${fileName} -- ${executable} -r ${rootPath} -j ${packageJsonPath} --package-json-dependencies-only`,
      );
      return;
    }
  ...
  _generateIndexJs(autogeneratedDir) {
    ...
    fs.writeFileSync(
      ...
      `require('../${engineParent}/engine/index')`,
    );
  }
}
