...
const IOSBuildFlavors = {
  dev: {scheme: 'engine', config: 'Debug', flavorDir: 'Debug'},
};

class IOSBuilder {
  constructor() {
    this._repoDir = `${__dirname}/../../../../..`;
    this._engineIosDir = `${this._repoDir}/internal_folder/engine/ios`;
    this._workspacePath = `${this._engineIosDir}/engine.xcworkspace`;
  ...
  _buildiOSBinary(platform, buildType, version) {
    ...
    const buildCommand = `RCT_NO_LAUNCH_PACKAGER=true xcodebuild \
      ...
      -derivedDataPath ${this._engineIosDir}/DerivedData/engine \
  ...
  _copyToAppBuilds(platform, buildType, buildFlavor) {
    const source = `${this._engineIosDir}/DerivedData/engine/build/Products/${buildFlavor.flavorDir}-${platform.name}/engine.${platform.ext}`;
    const destinationDir = `${this._repoDir}/internal_folder/engine/app_builds/${platform.name}/${buildType}`;
    execSync(`mkdir -p ${destinationDir}`);
    const destination = `${destinationDir}/engine.app`;
  ...
  _extractIpa(buildType, version) {
    ...
    const finalBinaryDirectory = `"${this._repoDir}/internal_folder/engine/app_builds/iphoneos/${buildType}"`;
    execSync(`mkdir -p ${finalBinaryDirectory}`);
    execSync(`cp -v "${tempDir}/${buildFlavor.scheme}.ipa" ${finalBinaryDirectory}/engine.ipa`);
    execSync(`rm -rf ${tempDir}`);
    return `${finalBinaryDirectory}/engine.ipa`;
  }
