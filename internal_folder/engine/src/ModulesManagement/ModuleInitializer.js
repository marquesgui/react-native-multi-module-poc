...
export class ModuleInitializer {
  constructor({ sendBroadcast, registerMethod, registerComponent, registerListener, removeListener, registerConsumedServices, modulesConfig }) {
  ...
  init() {
    forEach(this.modulesOrder, moduleName => {
      try {
        ...
        if (module.tabs) {
          forEach(module.tabs(), tab => {
            TabScreens.push(tab);
          });
        }

        if (module.components) {
          forEach(module.components(), this._getComponentRegistratorForModule(moduleName, module));
        }

        if (module.methods) {
          forEach(module.methods(), this._getMethodRegistratorForModule(moduleName, module));
        }
        ...
        if (module.listeners) {
          ...
          forEach(module.listeners(remover), this._getListenerRegistratorForModule(module.prefix(), remover));
        }
    ...
    if (module.providedServices) {
      try {
        ...
        forEach(Object.keys(providers), providerName => {
          ...
          providers[providerName](getServiceConsumers(this.consumedServices, providerName));
        });
      } catch (error) {
        if (__DEV__) {
          throw error;
          ...
          console.error(`FATAL ERROR, cannot init module ${module.prefix()} providers`, error);
        }
      }
    }
  }
  ...
  _checkValidPrefixForId(moduleName, module, id) {
    if (__DEV__) {
      if (!startsWith(id, module.prefix())) {
        console.error(`Module: ${moduleName} registers Id: ${id} with wrong prefix. (expected prefix: '${module.prefix()}')`);
      }
    }
  }
}
