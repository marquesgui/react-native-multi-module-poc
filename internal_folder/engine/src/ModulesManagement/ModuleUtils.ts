...
export function getServiceConsumers(consumedServices: ConsumedServices, serviceName: string) {
    ...
    for (let i = 0; i < consumersInfo.length; i++) {
      ...
      if (info.value) {
        ...
        try {
          ...
          console.warn(`cannot run generator for service ${serviceName} of module ${module.prefix()}`, e);
        }
      }
    }
  }
